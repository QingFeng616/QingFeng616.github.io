...
### BMS 开发日记
 ...
 6. 电池均衡 (*已处理*)
    1. 电池均衡使能
      芯片手册规定不能同时启动相邻两个电池单元的均衡功能,否则会导致均衡功能失效!!!
      ```markdown
      解决方案:选择分为奇数组和偶数组,分别使能,避免相邻电池单元的均衡功能冲突。
      ```
    2. 电池均衡失能
      注意:电池均衡使能后需要在均衡结束失能指定电芯均衡,均衡相关函数有修改,注意检查!!!
 
 7. 系统状态寄存器的特殊配置(*已处理*)(待处理-读取后标志位清0)
 
     完整逐段讲解（TzhidoI BQ769xx 电池AFE芯片 SYS_STAT 状态寄存器 0x00）
    #### 一、基础表格总览
    寄存器名称：**SYS_STAT**
    寄存器地址：**0x00**
 ...
 9. BQ76920初始化流程(*已处理*)
 ```
 问题描述:初始化流程详见 项目相关-BMS电池管理系统-上电流程
 ```
 10. ALERT引脚响应(*已处理*)
 
 | 故障类型 | CHG_FET | DSG_FET |
| --- | --- | --- |
| OV（过压） | ❌ 自动关闭 | 不变 |
| UV（欠压） | 不变 | ❌ 自动关闭 |
| OCD（过流） | 不变 | ❌ 自动关闭 |
| SCD（短路） | 不变 | ❌ 自动关闭 |
| OVRD_ALERT（外部覆盖） | ❌ 自动关闭 | ❌ 自动关闭 |
| DEVICE_XREADY | ❌ 自动关闭 | ❌ 自动关闭 |
| CC_READY（仅数据就绪） | 不变 | 不变 |
11. 查看电流对应电阻阻值设置
